...
_Комментарий_: pop_back() за O(1) работает благодаря указателя на tail, без него было бы за 
O(n);
...
---
### Примеры вопросов:
- Что такое двусвязный список? Какие элементы он в себе содержит?
- `+` и `-` двусвязного списка по сравению с массивом и односвязным списком?
- Стандартные операции на двусвязном списке. Их ассимптотика.
- Для чего нужны указатели на предыдущий и следующий элемент?
- Что такое `head` и `tail`?
- Можно ли реализовать двусвязный список без `tail`?
- Почему двусвязный список требует больше памяти?
- Какие ошибки возможны при неправильном обновлении ссылок?
- Реализация очереди на двусвязном списке.
- Как происходит вставка узла?
